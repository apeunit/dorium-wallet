...
let tokenInfo;
let valueTokenContract = "wasm1unyuj8qnmygvzuex3dwmg9yzt9alhvye40szph";
...
window.onload = async () => {
  ...
  if (qparams.has("contract")) {
    valueTokenContract = qparams.get("contract");
  }

  if (!valueTokenContract) {
  ...
  try {
    tokenInfo = await cosmJS.queryContractSmart(valueTokenContract, {
      token_info: {},
    });
...
document.sendForm.onsubmit = () => {
  ...
  (async () => {
    ...
    try {
      const fee = await calculateFee(10000000, "0.001udor");
    ...
    if (window.keplr.experimentalSuggestChain) {
      try {
        ...
        await window.keplr.experimentalSuggestChain({
          ...
          currencies: [
            { coinDenom: "TREE", coinMinimalDenom: "utree", coinDecimals: 6 },
            { coinDenom: "SOBZ", coinMinimalDenom: "usobz", coinDecimals: 6 },
            {
              // Coin denomination to be displayed to the user.
              coinDenom: "DOR",
              // Actual denom (i.e. uatom, uscrt) used by the blockchain.
...
async function updateBalance(address, client) {
  ...
  const result = await client.queryContractSmart(valueTokenContract, qBalance);
